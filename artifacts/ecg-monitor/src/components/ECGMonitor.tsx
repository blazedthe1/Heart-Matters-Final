...
export default function ECGMonitor() {
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      ctx.strokeStyle = `rgba(${hexToRgb(cond.color)}, 0.04)`;
      ctx.lineWidth = 0.5;
      ...
      ctx.strokeStyle = `rgba(${hexToRgb(cond.color)}, 0.08)`;
      ctx.lineWidth = 1;
      for (let i = 0; i <= NUM_GRID_H; i++) {
        ctx.beginPath(); ctx.moveTo(i * cellW * 5, 0); ctx.lineTo(i * cellW * 5, H); ctx.stroke();
      }
      for (let j = 0; j <= NUM_GRID_V; j++) {
        ctx.beginPath(); ctx.moveTo(0, j * cellH * 5); ctx.lineTo(W, j * cellH * 5); ctx.stroke();
      }
      ...
      for (let i = 0; i < W; i++) {
        ...
        points.push({ x: i, y: H / 2 - y * (H * 0.38) });
      }

      for (let pass = 0; pass < 3; pass++) {
        ctx.shadowBlur = [12, 6, 2][pass];
        ctx.shadowColor = cond.glowColor;
        ctx.strokeStyle = `rgba(${hexToRgb(cond.color)}, ${[0.18, 0.3, 1][pass]})`;
      ...
      const nearestIdx = Math.min(Math.floor(mouseXRef.current * (points.length - 1)), points.length - 1);
      const dotY = points[nearestIdx]?.y ?? H / 2;

      ctx.strokeStyle = `rgba(${hexToRgb(cond.color)}, 0.25)`;
      ctx.lineWidth = 1;
      ctx.setLineDash([4, 4]);
      ...
      ctx.beginPath(); ctx.arc(cursorX, dotY, 5, 0, Math.PI * 2); ctx.fill();
      ctx.shadowBlur = 0;
    ...
    >
      ...
      <div className="relative z-10 w-full flex flex-col items-center gap-6 px-6">
        ...
        <div
          ...
          style={{
            border: `1px solid rgba(${hexToRgb(condition.color)}, 0.25)`,
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="w-px self-stretch opacity-20" style={{ backgroundColor: condition.color }} />
            ...
            {conditions.map((c, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span
                  ...
                  style={{
                    color: i === conditionIndex ? c.color : `rgba(${hexToRgb(c.color)}, 0.35)`,
